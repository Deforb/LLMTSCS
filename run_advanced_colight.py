...
def parse_args():
    ...
    parser.add_argument(
        "--traffic_file", type=str, default="anon_28_7_newyork_real_triple.json"
    )
...
def main(in_args=None):
    ...
    deploy_dic_agent_conf = merge(
        getattr(config, "DIC_BASE_AGENT_CONF"), dic_agent_conf_extra
    )
    ...
    deploy_dic_traffic_env_conf = merge(
        config.dic_traffic_env_conf, dic_traffic_env_conf_extra
    )
