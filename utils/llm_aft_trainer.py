...
def copy_cityflow_file(dic_path, dic_traffic_env_conf, path=None):
    if path is None:
        path = dic_path["PATH_TO_WORK_DIRECTORY"]
    shutil.copy(
        os.path.join(dic_path["PATH_TO_DATA"], dic_traffic_env_conf["TRAFFIC_FILE"]),
        os.path.join(path, dic_traffic_env_conf["TRAFFIC_FILE"]),
    )
    shutil.copy(
        os.path.join(dic_path["PATH_TO_DATA"], dic_traffic_env_conf["ROADNET_FILE"]),
...
class LLM_CGPR_Collector:
    ...
    def initialize_critic(self):
        ...
        dic_critic_agent_conf = merge(
            dic_adv_colight_agent_conf_extra, config.DIC_BASE_AGENT_CONF
        )

        if (
            dic_critic_agent_conf["MODEL_NAME"]
            in self.dic_traffic_env_conf["LIST_MODEL_NEED_TO_UPDATE"]
        ):
        ...
        compare_dic_traffic_env_conf["LIST_STATE_FEATURE"] = dic_critic_agent_conf[
            "LIST_STATE_FEATURE"
        ]
        ...
        for i in range(self.dic_traffic_env_conf['NUM_AGENTS']):
            critic_agents[i].load_network(
                f'round_{round_num}_inter_0',
                file_path=f"./model_weights/{dic_critic_agent_conf['MODEL_NAME']}/{traffic_file}/",
            )
    ...
    def collect(self):
        ...
        for step_num in tqdm(
            range(int(total_run_cnt / self.dic_traffic_env_conf['MIN_ACTION_TIME']))
        ):
            ...
            for i in range(response_ids.size(0)):
                responses.append(
                    self.tokenizer.batch_decode(
                        response_ids[i], skip_special_tokens=True
                    )
                )
            ...
            for i, res in enumerate(responses):
                action_response = res[random.randint(0, 3)][len(prompts[i]) :]
...
class LLM_CGPR_Trainer:
    ...
    def initialize_llm(self):
        ...
        self.training_args = TrainingArguments(
            output_dir=f"{self.dic_agent_conf['LLM_OUTPUT_DIR']}_{self.dic_traffic_env_conf['TRAFFIC_FILE'].replace('.json', '')}",
    ...
    def train(self):
        ...
        data = load_dataset(
            "json",
            data_files=f"./data/cgpr/cgpr_{self.dic_agent_conf['CGPR_DATA_PATH']}",
        )

        train_val = data["train"].train_test_split(
            test_size=0.1, shuffle=True, seed=2024
        )
        ...
        data_module = make_supervised_data_module(
            ...
            train_data,
            val_data,
            mix=True if 'mix' in self.dic_agent_conf['CGPR_DATA_PATH'] else False,
        )
        self.trainer = RankTrainer(
    ...
    def test(self, logger, test_round):
        ...
        for step_num in tqdm(
            ...
            response_ids = self.llm_model.generate(
                input_ids=inputs["input_ids"], **self.test_generation_kwargs
            )
        ...
        results = {
            ...
            "env/test_avg_queue_len": (
                np.mean(queue_length_episode) if len(queue_length_episode) > 0 else 0
            ),
            "env/test_queuing_vehicle_num": (
                np.sum(queue_length_episode) if len(queue_length_episode) > 0 else 0
            ),
            "env/test_avg_waiting_time": (
                np.mean(waiting_time_episode) if len(queue_length_episode) > 0 else 0
            ),
    ...
    def train_test(self):
        ...
        all_config = merge(
            merge(self.dic_agent_conf, self.dic_path), self.dic_traffic_env_conf
        )
        logger = wandb.init(
        ...
        for step_num in tqdm(
            ...
            for i in range(len(current_states)):
                if (i + 1) % 16 == 0 or i + 1 >= len(current_states):
                    response_ids = self.llm_model.generate(
                        input_ids=inputs["input_ids"][previous_flag : i + 1],
                        **self.test_generation_kwargs,
                    )
                    responses += self.tokenizer.batch_decode(
                        response_ids, skip_special_tokens=True
...
class LLM_Inference_VLLM:
    ...
    def initialize_llm(self):
        ...
        self.llm_model = vllm.LLM(
            model=llm_path, tokenizer=llm_path, dtype=torch.bfloat16
        )
            ...
            for i in range(len(current_states)):
                if (i + 1) % 16 == 0 or i + 1 >= len(current_states):
                    responses_meta = self.llm_model.generate(
                        prompts=prompts[previous_flag : i + 1],
