    ...
    def generate(self, logger):
        ...
        for step_num in tqdm(
            ...
            for i in range(self.dic_traffic_env_conf["NUM_AGENTS"]):
                ...
                if self.dic_traffic_env_conf["MODEL_NAME"] in [
                    "EfficientPressLight",
                    "EfficientColight",
                    "EfficientMPLight",
                    "Attend",
...
class Generator_LLMLight:
    ...
    def generate(self, logger, tokenizer, llm_model, generation_kwargs):
        ...
        for step_num in tqdm(
            ...
            print(
                "time: {0}, running_time: {1}, fail_num".format(
                    self.env.get_current_time()
                    - self.dic_traffic_env_conf["MIN_ACTION_TIME"],
                    time.time() - step_start_time,
                    fail_num,
