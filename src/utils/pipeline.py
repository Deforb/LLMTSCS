...
def generator_wrapper(
    cnt_round, cnt_gen, dic_path, dic_agent_conf, dic_traffic_env_conf, logger
):
    generator = Generator(
        cnt_round=cnt_round,
        cnt_gen=cnt_gen,
...
class Pipeline:
    ...
    def run(self, round, multi_process=False):
        f_time = open(
            os.path.join(self.dic_path["PATH_TO_WORK_DIRECTORY"], "running_time.csv"),
            "w",
        )
        f_time.write(
        ...
        for cnt_round in range(self.dic_traffic_env_conf["NUM_ROUNDS"]):
            ...
            making_samples_total_time = (
                making_samples_end_time - making_samples_start_time
            )

            print("==============  update network =============")
            update_network_start_time = time.time()
            if (
                self.dic_traffic_env_conf["MODEL_NAME"]
                in self.dic_traffic_env_conf["LIST_MODEL_NEED_TO_UPDATE"]
            ):
                if multi_process:
                    p = Process(
                        target=updater_wrapper,
                        args=(
                            cnt_round,
                            self.dic_agent_conf,
                            self.dic_traffic_env_conf,
            ...
            update_network_total_time = (
                update_network_end_time - update_network_start_time
            )

            print("==============  test evaluation =============")
            test_evaluation_start_time = time.time()
            results, state_action_log = model_test.test(
                self.dic_path["PATH_TO_MODEL"],
                self.dic_path["PATH_TO_DATA"],
                cnt_round,
            ...
            print(
                "round {0} ends, total_time: {1}".format(
                    cnt_round, time.time() - round_start_time
                )
            )
            f_time = open(
                os.path.join(
                    self.dic_path["PATH_TO_WORK_DIRECTORY"], "running_time.csv"
                ),
                "a",
            )
            f_time.write(
                "{0}\t{1}\t{2}\t{3}\t{4}\n".format(
                    generator_total_time,
                    making_samples_total_time,
                    update_network_total_time,
                    test_evaluation_total_time,
        ...
        last_10_results = {
            ele: np.mean(last_10_results[ele]) for ele in last_10_results
        }
