...
class OneLine:
    ...
    def train(self, round):
        ...
        file_name_memory = os.path.join(
            self.dic_path["PATH_TO_WORK_DIRECTORY"], "memories.txt"
    )
        done = False
        ...
        while not done and current_time < total_run_cnt:
            ...
            for i, one_state in enumerate(state):
                ...
                count = step_num
                if (
                    "ChatGPT" in self.dic_traffic_env_conf["MODEL_NAME"]
                    or "open_llm" in self.dic_traffic_env_conf["MODEL_NAME"]
                ):
                    thread = threading.Thread(
                        target=self.agents[i].choose_action, args=(self.env,)
                    )
            ...
            if "open_llm" in self.dic_traffic_env_conf["MODEL_NAME"]:
                started_thread_id = []
                thread_num = (
                    self.dic_traffic_env_conf["LLM_API_THREAD_NUM"]
                    if not self.dic_agent_conf["WITH_EXTERNAL_API"]
                    else 2
                )
            ...
            memory_str = (
                'time = {0}\taction = {1}\tcurrent_phase = {2}\treward = {3}'.format(
                    current_time,
                    str(action_list),
